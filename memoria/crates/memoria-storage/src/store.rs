...
impl SqlMemoryStore {
    ...
    /// Create a store backed by an existing pool (reuse DbRouter's shared_pool).
    pub fn from_existing_pool(pool: MySqlPool, database_url: &str, embedding_dim: usize, instance_id: String) -> Self {
    ...
    pub async fn count_snapshot_registrations(&self, user_id: &str) -> Result<i64, MemoriaError> {
        ...
        sqlx::query_scalar(&format!(
            "SELECT COUNT(*) FROM {snapshots_table} WHERE user_id = ? AND status = 'active'"
        ))
        .bind(user_id)
        .fetch_one(&self.pool)
        .await
